...
# Importing all images in a given folder for the animation
def importing(given_path, width, height):
    extracting_list = []

    for _,__,images in walk(given_path):
        for image in images:
            actual_path = given_path + '/' + image
            image_pic = pg.transform.scale(pg.image.load(actual_path), (width, height)).convert_alpha()
            extracting_list.append(image_pic)

    return extracting_list

class Buttons:
    def __init__(self):
        # Regular buttons
        self.play_image = pg.transform.scale(pg.image.load('Data/PlayButton.png'), (100, 25))
        self.quit_image = pg.transform.scale(pg.image.load('Data/QuitButton.png'), (100, 25))
        self.play_button = self.play_image.get_rect()
        self.quit_button = self.play_image.get_rect()
        self.play_button.center = (150, 200)
        self.quit_button.center = (150, 250)

        # Highlighted buttons
        self.play_image_hl = pg.transform.scale(pg.image.load('Data/PlayButtonHighlight.png'), (100, 25))
        self.quit_image_hl = pg.transform.scale(pg.image.load('Data/QuitButtonHighlight.png'), (100, 25))
        self.play_button_hl = self.play_image_hl.get_rect()
        self.quit_button_hl = self.play_image_hl.get_rect()
        self.play_button_hl.center = (150, 200)
        self.quit_button_hl.center = (150, 250)
...
class Animation:
    def __init__(self):
        ...
        self.image = self.animations['Animation'][self.frame_index]
        self.alien = self.image.get_rect()
        self.alien.center = (450, 200)
        self.anim1 = True
    ...
    def menu_animation(self):
        ...
        for animation in self.animations.keys():
            actual_path = sending_path + animation
            self.animations[animation] = importing(actual_path, self.alien_width, self.alien_height)

    def animate(self):
        # Default animation
        animation = self.animations['Animation']

        # Cycling images / actual animation
        if self.anim1:
...
def main(winstyle=0):
    ...
    menu_bg = pg.image.load('Data/AlienMenyBakgrund.png')
    ...
    if pg.mixer:
        menu_music = os.path.join(main_dir, "data", "MenuSong.mp3")
    ...
    while menu_selection:
        ...
        screen.blit(anim.image, (anim.alien.x, anim.alien.y))

        if button.play_button.collidepoint(mouse_pos):
            screen.blit(button.play_image_hl, (button.play_button_hl.x, button.play_button_hl.y))
            if mouse_pressed[0]:
                play = True
        else:
            screen.blit(button.play_image, (button.play_button.x, button.play_button.y))

        if button.quit_button.collidepoint(mouse_pos):
            screen.blit(button.quit_image_hl, (button.quit_button_hl.x, button.quit_button_hl.y))
            if mouse_pressed[0]:
                return
        else:
            screen.blit(button.quit_image, (button.quit_button.x, button.quit_button.y))
